...
function hitsRenderer(params, thd) {
    // Extract reason code from "0 (EUR)" format
    var reasonMatch = typeof params.value === 'string' && params.value.match(/^0 \(([A-Z]+)\)$/);
    var reasonCode = reasonMatch ? reasonMatch[1] : null;
    var exclusion = {
        hasComment: params.data.c != undefined || params.data.exc != undefined ,
        excludedStatus: isValueExcluded(params),
        hasReason: reasonCode ? GLOBAL_JSON.exclReason[reasonCode] : GLOBAL_JSON.exclReason[params.value],
        isZeroWithReason: !!reasonCode
    };

    //Testhit Data Renderer
    if (params.data[thd.fileNum] && params.data[thd.scope] !== undefined &&
        (params.value != 0 && !exclusion.isZeroWithReason)) { // testhitdata should not be shown if hits == 0 or if it is a zero with reason
...
function exclRenderer(params, exclusion) {
    var exclusionText = exclusion.isZeroWithReason ? params.value :
                       (params.value === 'E-hit' ? 'Excluded-Hit' : 'Excluded');
    var exclusionIcon = ((exclusion.hasComment && exclusion.excludedStatus) || exclusion.hasReason);
    var exclusionPopover = '';

    if (exclusion.isZeroWithReason) {
        exclusionPopover = 'Miss reason: ' + HTMLEncode(exclusion.hasReason);
    } else {
        if (params.data.c) {
            exclusionPopover = 'Exclusion comment: ' + HTMLEncode(params.data.c);
        } else if (params.data.exc) {
            exclusionPopover = 'Exclusion comment: ' + HTMLEncode(params.data.exc);
        }
        if (exclusion.hasReason) {
            if (exclusionPopover)  exclusionPopover += '\n';
            exclusionPopover += 'Exclusion reason: ' + HTMLEncode(exclusion.hasReason);
        }
    }
    ...
    var cellTemplate = $(
        ...
        (exclusionIcon ? //Adding icon in case of exclusion comment or exclusion reason
            ('<i class="fa fa-info-circle" aria-hidden="true" data-toggle="popover" title="' + exclusionPopover + '" style="color:#2361a0" id="item' + params.rowIndex + '"></i></div>') : ('</div>'))
    );
