...
var zeroWithReasonPattern = /^0 \([A-Z]+\)$/;
...
function processGlobaljson() {
    ...
    GLOBAL_JSON = {
        ...
        showuntested: g_oCONST.hasOwnProperty('commandargs') ? (checkCommandArgs(g_oCONST.commandargs, '-untested') && checkCommandArgs(g_oCONST.commandargs,'-details')) : undefined,						                        // isuntested
...
// Don't consider as excluded if it matches "0 (EUR)" pattern
function isValueExcluded(params) {
    return (typeof params.value === 'string' && params.value !== '-' && !zeroWithReasonPattern.test(params.value));
}

function isExcluded(params) {
    return (typeof params.data.hits === 'string' && !zeroWithReasonPattern.test(params.data.hits));
}

function isCondExprExcluded(params) {
    if ((typeof params.data.hits === 'string' && zeroWithReasonPattern.test(params.data.hits)) ||
        (typeof params.data.hitsTo0 === 'string' && zeroWithReasonPattern.test(params.data.hitsTo0)) ||
        (typeof params.data.hitsTo1 === 'string' && zeroWithReasonPattern.test(params.data.hitsTo1))) {
        return false;
    }
...
function isTogglesExtendedValueExcluded(params) {
    return (params.value === 'E' || params.value === 'E-hit' || params.data.coverage === 'E' ||
        (typeof params.data.hits === 'string' && !zeroWithReasonPattern.test(params.data.hits)));
}
...
function gridToolbarTemplate(id, columnDefs, nEntries, options) {
    ...
    if (gridOptionsList[gridId].pagination) {
        controlPanelTemplate +=
   			...
   			'<option value="1000" ' + ((options.pageSize === 1000) ? 'selected' : '') + '>1000</option>' +
			'<option value="2000" ' + ((options.pageSize === 2000) ? 'selected' : '') + '>2000</option>' +
