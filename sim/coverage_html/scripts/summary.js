...
$(document).ready(function () {
    ...
    isShowUntested = (GLOBAL_JSON.hasOwnProperty('showuntested') && GLOBAL_JSON.showuntested);
    DATA_ARR = {
    	BINS_INDEX: 0,
    	HITS_INDEX: 1,
    	EXCLUDED_INDEX: (isShowExcluded || isShowUntested) ? 2 : undefined,
    	FILE_NUM_INDEX: (isShowExcluded || isShowUntested) ? 3 : 2,
    	FILE_SUB_NUM_INDEX: (isShowExcluded || isShowUntested) ? 4 : 3
    };
    // parse url
    urlParams = parseUrl();
        ...
        if (data.hasOwnProperty(covType)) {
            if (!(covType === 'gb' || covType === 'cvpc' || covType === 'f' || covType === 'pb')) {
                    var rowData = {
                        ...
                        exclude: (isShowExcluded || isShowUntested) ?  data[covType][DATA_ARR.EXCLUDED_INDEX] : -1
                    };

                    if (data[covType].length > 3 || (data[covType].length > 2 && !(isShowExcluded || isShowUntested))) {
...
function covSummaryTables(id, dataObj, type) {
    ...
    if ((isShowExcluded || isShowUntested)) {
        columnDefs.push(
            {
                headerName: 'Excluded',
