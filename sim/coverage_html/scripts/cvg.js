...
function processCovergroupsData(g_data) {
    ...
    for (let key in g_data) {
        if (g_data.hasOwnProperty(key)) {
            const entry = g_data[key];
            if (entry.cvgs) {
                entry.cvgs.forEach(cvg => {
                    if (cvg.h && typeof cvg.h[0] === "string") {
                        cvg.h[0] = escapeAngleBrackets(cvg.h[0]);
                    }
                });
            }
        }
...
function getData(dataObj) {
    ...
    for (var key in dataObj) {
        ...
        if (dataObj.hasOwnProperty(key) && !( dataObj[key].ty == 2 && urlParams.type === 'du' && key != dataObj[key].dui && data.hasOwnProperty(dataObj[key].dui))) {
...
NameCellRenderer.prototype.init = function(params) {
    ...
    // Adding Comment Params
    if (params.data.opts && params.data.opts.comment) {
        href += (href.length ? '&'  : '') +'CVPComment=' +params.data.opts.comment;
    }
            ...
            if (params.data.type === COVERGROUPS_IDs.COVERPOINT || params.data.type === COVERGROUPS_IDs.CROSS) {
                switch (e.which) {
                    case 1:
                        ...
                        if (typeof href !== 'undefined') {
                            ...
                            // Adding Comment Params
                            if (params.data.opts && params.data.opts.comment) {
                                binUrlParams += (binUrlParams.length ? '&'  : '') +'CVPComment=' +params.data.opts.comment;
                            }
                ...
                if (e.which == 1) {
                    ...
                    if (params.data.sc) {   // Source file  Exists
                        headerTemplate += '<a href="sourceViewer.html?f='+ params.data.sc[0] + '&l='+ params.data.sc[1]+ '">' +  escapeAngleBrackets(e.currentTarget.innerText || e.currentTarget.innerHTML) + '</a>'
                    } else {
                        headerTemplate += escapeAngleBrackets(e.currentTarget.innerText || e.currentTarget.innerHTML)
                    }
                    ...
                    var summaryComment = params.data ? params.data.comment: '';
                    if (summaryComment) {
                        $('#summaryCommentText').text(summaryComment)
                    } else{
                        $('#summaryComment').hide()
                    }
    ...
    var templates = {
        ...
        'details':
            '<div id="details-container" class="col-xs-12" style="display:none; height:calc(100% - 10px);">' +
                ...
                '<h5 id="summaryComment" style="background: #ddd;padding: 10px;"><strong>Comment : </strong> <span id="summaryCommentText"></span> </h5>'+
                '<br /> '
        ,
